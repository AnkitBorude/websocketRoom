...
import { ChatMessage, CreateMessage, JoinMessage, RenameMessage, RoomNotificationMessage } from "./types";
type Client={
  id:number,
  name:string,
  ws:WebSocket
...
export class RoomManager{
    ...
    {
        this.clientStart++;
        const client= {id:(this.clientStart),name:"User "+this.clientStart,ws};
    ...
    createRoom(name:string,creator:Client)
    {
        ...
        const response = this.messageFactory(RequstType.CREATE,"Room Created Successfully your are part of your room")(name,this.roomIdStart);
        creator.ws.send(JSON.stringify(response));
    }
    ...
    renameUser(ws:WebSocket,newUsername:string)
    {
        ...
        if(!client)
        {
            const response=this.messageFactory(RequstType.RENAME,"Client not found 404")(newUsername);
            ws.send(JSON.stringify(response));
            return;
        }

        client.name=newUsername;
        const response=this.messageFactory(RequstType.RENAME,"Username Changed")(newUsername);
        ws.send(JSON.stringify(response));
        return;
    }
    ...
    joinRoom(ws:WebSocket,roomId:number)
    {
        ...
        {
            const response=this.messageFactory(RequstType.JOIN,"Room NOT Found 404")(roomId,client.name);
            ws.send(JSON.stringify(response));
            return;
        }

        const roomToJoin=this.chatRooms.get(roomId);
        const JoinNotificationToOthers:RoomNotificationMessage={type:"notify",message:`${client.name} has Joined the Room`,notificationOf:RequstType.JOIN};
        const JoinNotificationToUser:RoomNotificationMessage={type:'notify',message:`Joined to room ${roomToJoin?.name} current Online ${roomToJoin?.clients.length}`,notificationOf:RequstType.JOIN}
   ...
   // Overload signatures
private messageFactory(request: RequstType.CREATE,message:string): (roomName: string, roomId: number) => CreateMessage;
private messageFactory(request: RequstType.JOIN,message:string): (roomId: number, username: string) => JoinMessage;
private messageFactory(request: RequstType.MESSAGE,message:string): (roomId: number, message: string) => ChatMessage;
private messageFactory(request: RequstType.RENAME,message:string): (username: string) => RenameMessage;
